...
namespace FPVenturesHAWXIntegration.Services
{
	public class ZohoLeadsService : IZohoLeadsService
	{
		...
		public List<Data> GetZohoLeads(string dateTime, ILogger logger)
		{
			...
			zohoCOQLModel.Query = string.Format(_zohoHAWXConfigurationSettings.COQLQuery, dateTime);
			...
			try
			{
				using (var client = new HttpClient())
				{
					...
					var zohoLeadsModel = JsonConvert.DeserializeObject<ZohoLeadsModel>(result);
