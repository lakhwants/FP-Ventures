using FPVenturesZohoInventoryBills.Constants;
using FPVenturesZohoInventoryBills.Models;
using System;
using System.Collections.Generic;
using System.Linq;

namespace FPVenturesZohoInventoryBills.Services.Mapper
{
    public class ModelMapper
    {
        public static List<ZohoInventoryBillsModel> MapBillsModel(List<InventoryItem> inventoryItems, List<VendorCRM> vendorsCRM, List<VendorInventory> vendorsInventory, DateTime startDate, DateTime endDate)
        {
            MapItemsWithVendors(inventoryItems, vendorsCRM, vendorsInventory);
            List<ZohoInventoryBillsModel> zohoInventoryBillsModels = new();

            var itemGroupsByVendors = inventoryItems.Where(y => y.PurchaseRate != 0).GroupBy(x => x.VendorName).ToList();
