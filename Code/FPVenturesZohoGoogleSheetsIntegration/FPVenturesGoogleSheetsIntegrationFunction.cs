using FPVenturesZohoGoogleSheetsIntegration.Constants;
...
namespace FPVenturesZohoGoogleSheetsIntegration
{
	public class FPVenturesGoogleSheetsIntegrationFunction
	{
		  ...
		  FunctionContext executionContext)
		{
			...
			string csvString;
			var dateTime = DateTime.Now.Date.AddDays(-10).ToString("yyyy-MM-dd");

			logger.LogInformation($"Date {dateTime}");

			logger.LogInformation("Getting report from Zoho analytics");

			var conversionReportsModel = _zohoAnalyticsService.GetReports(dateTime, reportName);
			...
			if (reportName == ReportNames.RingbaConversionReport)
				csvString = csvString.Replace("Callers", "Caller's");

			logger.LogInformation($"Execution Finished");

			response.WriteString(csvString);
			return response;
		}
	}
}
