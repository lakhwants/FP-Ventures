...
namespace FPVenturesRingbaZohoInventory.Services.Mapper
{
    public class ModelMapper
    {
        /// <summary>
        /// Maps Ringba calls to Inventory items
        /// </summary>
        /// <param name="ringbaCallLogsGroups">Groups of Ringba calls by publishers</param>
        /// <param name="zohoInventoryItemGroupsList">ZOHO Inventory item groups list model </param>
        /// <param name="zohoInventoryVendorsResponseModel">ZOHO Inventory vendor model</param>
        /// <param name="zohoCRMVendorsResponseModel">ZOHO CRM vendor model</param>
        /// <returns></returns>
        public static List<List<ZohoInventoryModel>> MapRingbaCallsToZohoInventoryItems(List<IGrouping<string, Record>> ringbaCallLogsGroups, ZohoInventoryItemGroupsListResponseModel zohoInventoryItemGroupsList, ZohoInventoryVendorsResponseModel zohoInventoryVendorsResponseModel, ZohoCRMVendorsResponseModel zohoCRMVendorsResponseModel, ILogger logger)
        {
                ...
                foreach (var ringbaCallLog in ringbaCallLogs)
                {
                    ...
                    try
                    {
                        ...
                        List<CustomField> customFields = new()
                        {
                            ...
                            new CustomField { Label = ZohoInventoryCustomFields.CallerID, Value = ringbaCallLog.InboundPhoneNumber.RemoveCountryCode() },
                            ...
                            new CustomField { Label = ZohoInventoryCustomFields.CallDateTime, Value =ringbaCallLog.CallDt.ToZohoDateString() },
                            ...
                            new CustomField { Label = ZohoInventoryCustomFields.TargetNumber, Value = ringbaCallLog.TargetNumber.RemoveCountryCode() },
        ...
        /// <summary>
        /// Maps ZOHO Inventory Items groups 
        /// </summary>
        /// <param name="newItemGroups">List of itemgroups by publishers</param>
        /// <returns></returns>
        public static List<ZohoInventoryPostItemGroupRequestModel> MapNewItemGroups(List<IGrouping<string, Record>> newItemGroups)
        {
            ...
            //adding a Demo item as it is mandatory to add an item while creating an item group
            //When we create an item group, at that time not custom field field for any item is allowed
        ...
        public static string CreateSKU(Record ringbaCallLog, VendorCRM vendor)
        {
            ...
            if (vendor == null)
                return "NA" + "_" + ringbaCallLog.InboundPhoneNumber.RemoveCountryCode() + "_" + ticks + "_" + Convert.ToString(TimeSpan.FromSeconds(ringbaCallLog.CallLengthInSeconds)) + "_" + ringbaCallLog.TaggedState;

            return vendor.VendorAbbreviation + "_" + ringbaCallLog.InboundPhoneNumber.RemoveCountryCode() + "_" + ticks + "_" + Convert.ToString(TimeSpan.FromSeconds(ringbaCallLog.CallLengthInSeconds)) + "_" + ringbaCallLog.TaggedState;
        }
    }
}
