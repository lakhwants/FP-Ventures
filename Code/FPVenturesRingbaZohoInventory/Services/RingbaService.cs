...
namespace FPVenturesRingbaZohoInventory.Services
{
    public class RingbaService : IRingbaService
    {
        ...
        public List<Record> GetCallLogDetails(List<Record> callLogs)
        {
            ...
            foreach (var record in callLogs)
            {
                ...
                foreach (var callRecord in callLogRecords)
                {
                    ...
                    record.IBNIsPhoneNumberValid = callRecord.GetMessageTagValue(RingbaCallLogDetails.IsPhoneNumberValid);
                    record.TaggedNumber = callRecord.GetMessageTagValue(RingbaCallLogDetails.Number);
                    record.TaggedState = callRecord.GetMessageTagValue(RingbaCallLogDetails.State);
                    record.TaggedLocationType = callRecord.GetMessageTagValue(RingbaCallLogDetails.LocationType);
                    record.TaggedNumberType = callRecord.GetMessageTagValue(RingbaCallLogDetails.NumberType);
                    record.TaggedTelco = callRecord.GetMessageTagValue(RingbaCallLogDetails.Telco);
                    record.TaggedCity = callRecord.GetMessageTagValue(RingbaCallLogDetails.City);
                    record.TaggedTimeZone = callRecord.GetMessageTagValue(RingbaCallLogDetails.TimeZone);
                    record.TaggedSRC = callRecord.GetMessageTagValue(RingbaCallLogDetails.SRC);
                }
            }
        ...
        /// <returns></returns>
        //private static string GetMessageTagValue(this CallLogDetail messageTag, string key)
        //{
